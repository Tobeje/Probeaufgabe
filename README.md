# Probeaufgabe

# MySQL Datenbank
In der sevdesk.sql Datei befindet sich ein SQL befehl um die Datenbank mit den Tabellen zu erstellen.

### Datenbankmodellierung

![Screenshot](datenbankmodellierung.PNG)

# Framework

Ich habe mich für das Framework Slim Version 3.12 entschieden

# Verwenden

Erstellen der Datenbank ggf. die src/config/db.php anpassen

Folgenden Befehl verwenden zum installieren der dependencies
```bash
composer install
```

Dann kann das Projekt auf einem Webserver verwendet werden